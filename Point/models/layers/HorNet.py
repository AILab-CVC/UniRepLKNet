...
if not hasattr(torch.fft, 'rfft2'):
    torch.fft.rfft2 = torch.fft.rfftn
    
if not hasattr(torch.fft, 'irfft2'):
    torch.fft.irfft2 = torch.fft.irfftn
...
@register_model
def hornet_large_gf(pretrained=False,in_22k=True, **kwargs):
    if in_22k:
        num_classes=10459
    else:
        num_classes=1000
    ...
    s = 1.0/3.0
    model = HorNet(depths=[2, 3, 18, 2], base_dim=192, block=Block, num_classes=num_classes,
    gnconv=[
        partial(gnconv, order=2, s=s),
        partial(gnconv, order=3, s=s),
        partial(gnconv, order=4, s=s, h=14, w=8, gflayer=GlobalLocalFilter),
