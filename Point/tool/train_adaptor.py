...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='config/ModelNet40/p2p_ConvNeXtV2-L-1k.yaml', help='config file')
    # parser.add_argument('--config', type=str, default='config/ModelNet40/p2p_ResNet-18.yaml', help='config file')
    # parser.add_argument('--config', type=str, default='config/ScanObjectNN/p2p_ResNet-18.yaml', help='config file')
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if args.data_name == 'modelnet':
        from dataset.modelnet import ModelNetFewShot, ModelNet
        ...
        from dataset.scanobjectnn import ScanObjectNNFewShot
        train_data = ScanObjectNNFewShot(config=args, subset='train')
        train_sampler = torch.utils.data.distributed.DistributedSampler(train_data,
                                                                        shuffle=True) if args.distributed else None
        train_loader = torch.utils.data.DataLoader(train_data, batch_size=args.batch_size,
                                                   shuffle=(train_sampler is None),
                                                   ...
                                                   drop_last=True, worker_init_fn=worker_init_fn)
        if args.evaluate:
            val_data = ScanObjectNNFewShot(config=args, subset='test')
            val_sampler = None
            val_loader = torch.utils.data.DataLoader(val_data, batch_size=args.batch_size_val,
                                                     shuffle=False, num_workers=args.workers, pin_memory=True,
                                                     drop_last=False, sampler=val_sampler)
        ...
        train_data = ShapeNetClsFewShot(data_root=args.data_root, num_points=args.npoints, partition='trainval', num_shots=args.num_shots)
        train_sampler = torch.utils.data.distributed.DistributedSampler(train_data,
                                                                        shuffle=True) if args.distributed else None
        train_loader = torch.utils.data.DataLoader(train_data, batch_size=args.batch_size,
                                                shuffle=(train_sampler is None),
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        is_best = False
        if (args.evaluate and (epoch_log % args.eval_freq == 0) and (epoch_log < args.epochs - args.last_epochs) and (epoch_log >= args.val_epoch)) or (
